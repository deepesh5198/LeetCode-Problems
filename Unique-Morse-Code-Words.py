...
def uniqueMorseCode(words):
    ...
    mapping = {key:val for key, val in zip(alphabets, morseList)}
    
    morse_representation = []
    for word in words:
        morse_representation.append("".join(mapping[letter] for letter in word))
        
    return len(set(morse_representation))
